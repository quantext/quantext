        ...
        load: function() {
            ...
            if( instance.options.search ) {
                ...
                search.on('keyup', function(e){
                    if(e.keyCode == 13) {
                        var pageInfo = getPageInfo(select);
                        var word = search.val();
                        pageInfo['word'] = word;
                        $.post("/add_category", pageInfo, function() {
                            search.val("");
                            var optionList = [{name:word,value:word,checked:false}];
                            var categories = $("#table_"+pageInfo["id"]+"_"+pageInfo["left_or_right"]+"_"+pageInfo["question"]+"_wrapper .categories").data("categories");
                            categories = (categories) ? categories.split(",") : [];
                            if(categories.indexOf(word) == -1)
                                categories.push(word);
                            $("#table_"+pageInfo["id"]+"_"+pageInfo["left_or_right"]+"_"+pageInfo["question"]+"_wrapper .categories").data("categories", categories.toString());
                            $("#table_"+pageInfo["id"]+"_"+pageInfo["left_or_right"]+"_"+pageInfo["question"]+"_wrapper").find('select[multiple]').multiselect('loadOptions',optionList,false);
                        });
                    }
                });
            }
            ...
            optionsWrap.on( 'click', 'input[type="checkbox"]', function(){
                ...
                if(isChecked)
                    $(this).closest( 'li' ).removeClass("special");

                var select = optionsWrap.parent().prev();
                var optValue = $(this).val();
                // toggle clicked option
                select.find('option[value="'+ optValue +'"]').prop(
                    'selected', isChecked
                ...
                var pageInfo = getPageInfo(select);
                var selectedRows = $("#table_" + pageInfo.id + "_" + pageInfo.left_or_right + "_" + pageInfo.question).DataTable().rows({selected:true});
                var IDS = [];
                var url = (isChecked) ? "/save_category" : "/remove_category";
                selectedRows.every( function () {
                    var cats = this.data()["Category"];
                    cats = cats ? cats.split(",") : [];
                    if(isChecked && cats.indexOf(optValue) == -1)
                        cats.push(optValue);
                    else if(!isChecked && cats.indexOf(optValue) != -1)
                        cats.splice(cats.indexOf(optValue), 1);

                    var data = this.data();
                    data["Category"] = cats.toString();
                    this.data(data).draw('full-hold');
                    IDS.push(this.data()["StudentID"]);
                });

                $.post(url, {id: pageInfo.id,question: pageInfo.question,response: JSON.stringify(IDS),category: optValue}, function () {
                    instance._updateSelectAllText();
                    instance._updatePlaceholderText();
                });
            });
