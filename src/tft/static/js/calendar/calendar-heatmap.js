function calendarHeatmap(opacities) {
    // defaults
    var width = 750;
    var height = 200;
    var legendWidth = 150;
    ...
    function chart() {
        ...
        function drawChart() {
            ...
            var enterSelection = dayRects.enter().append('rect')
                ...
                .attr('opacity', function(d) { return opacities(countForDate(d)); })
                .attr('x', function (d, i) {
            ...
            if (chart.legendEnabled()) {
                var colorRange = [.05,.3,.6,1];

                var legendGroup = g.append('g').attr("transform","translate(0,-80)");
                legendGroup.selectAll('.calendar-heatmap-legend')
                    .data(colorRange)
                    .enter()
                    .append('rect')
                    .attr('class', 'calendar-heatmap-legend day-cell')
                    .attr('width', SQUARE_LENGTH)
                    .attr('height', SQUARE_LENGTH)
            ...
            var monthLabels = g.selectAll('.month')
                .data(monthRange)
                .enter().append('text')
        ...
        function pluralizedTooltipUnit (count) {
            if ('string' === typeof tooltipUnit) {
                return (tooltipUnit + (count === 1 ? 'analysis' : 'analyses'));
            }
